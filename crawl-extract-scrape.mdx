...
This call will take about a minute, and the response will look something like this:
...
One thing you'll notice is that the `*` operator matches sub paths that we don't want, like `https://pokemondb.net/pokedex/game/legends-arceus`. You can use the `:*` operator instead, which will match everything _except_ the `/` character. The full pattern in that case would be `https://pokemondb.net/pokedex/:*` .
