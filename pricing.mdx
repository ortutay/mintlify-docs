...
FetchFox uses [cost plus pricing](https://en.wikipedia.org/wiki/Cost-plus_pricing), which means we charge a fixed fee on top of our underlying costs. The underlying costs for each scrape are the cost of network traffic through proxies, and the cost of AI usage. We pass on these costs directly, and add a fixed fee basd on API usage.

$$
Your\ Price = Underlying\ Cost + FetchFox\ Fees
$$
